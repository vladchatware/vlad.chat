...
export function registerComputerUseMcpTools(server: McpToolServer): void {
  ...
  server.tool(
    "computer_open",
    "Open a public URL in the isolated computer-use browser (Vercel Sandbox + Playwright). Returns JSON with screenshotUrl. Sessions capped at 8 min / 20 steps.",
    {
  ...
  server.tool(
    "computer_screenshot",
    "Capture the current computer-use browser viewport. Returns JSON with screenshotUrl for web and iOS.",
    {
      sessionId: sessionIdField,
    },
    async ({ sessionId }) => {
